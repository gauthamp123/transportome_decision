...
def writePrimarySubstrateTypes(tsvFile, newFile, delimiter=' '):
    try:
        newLines = [['#Types','Substrates','ChEBI']]
        with open(tsvFile) as file:
            tsv_file = csv.reader(file, delimiter="\t")
            header = next(tsv_file)
            data = {}
            for row in tsv_file:
                ...
                for id in row[2].split('/'):
                    entityIDs.append(findPrimary('CHEBI:'+ id, altIDToID))
                    cleanEntityIds.append(findPrimary('CHEBI:'+ id, altIDToID).split(':')[1])
                for primaryid in entityIDs:
                    data[primaryid] = {}
                    data[primaryid]['category'] = category
                    data[primaryid]['entityName'] = entityName
                ...
                newLines.append([category, entityName, '/'.join(cleanEntityIds)])

    
        with open(newFile, 'w', newline='') as tsvfile:
...
if __name__ == "__main__":
    ...
    print(writePrimarySubstrateTypes('substrateTypes.tsv', 'substrateTypesPrimary.tsv'))
