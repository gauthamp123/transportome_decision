...
def get_chebi_id(substrateData, tcids):
    ...
    for i in tcids:
        clean_tcid = i.split('-')[0]
        for key in substrateData:
            if key == clean_tcid:
                chebiIDs[key]=[item[0] for item in substrateData[key]]
        if clean_tcid not in chebiIDs:
            chebiIDs[clean_tcid] = 'none'
    return chebiIDs
...
def findPredecessor(id, ancdict, graph, altIDToID, idToName):
...
def findRole(id, id_to_relationship, idToName, altIDToID, graph, idToParent, classes = None, flag = True):
    ...
    if id == None or id_to_relationship[id] == None and flag == True:
        ...
        if id != None and id == 'CHEBI:26115' or id == 'CHEBI:26672' or id == 'CHEBI:27026' or id == 'CHEBI:72316':
            ...
            for validTerminal in classes:
                if validTerminal in allPredecessors and validTerminal != id:
    ...
    if id in classes and flag == False:
    ...
    if flag == True: 
        for entry in id_to_relationship[id]:
            if entry.split(' ')[0] == 'has_role':
                ...
                if targetID in classes and targetID != id:
                    roles.append(getSubstrateName(targetID, idToName) + '(' + targetID +')')
                
                elif targetID in id_to_relationship:
                    roles.extend(findRole(targetID, id_to_relationship, idToName, altIDToID, graph, idToParent, classes, False))
...
def terminalPredecessorParse(tsvFile, altIDToID, delimiter=' '):
    try:
