...
def main():
    ...
    for genome in os.listdir(pwd):
        ...
        gbff_files = [file for file in os.listdir(assembly_dir + '/' + genome) if file.endswith('.gbff.gz')]
        ifile = gbff_files[0]
        gbff_file = os.path.join(assembly_dir + '/' + genome, ifile)
        print(gbff_file)
        target_files = [file for file in os.listdir(assembly_dir + '/' + genome) if file.endswith('feature_table.txt.gz')]
        input_file_name = target_files[0]
        
        feature_table = os.path.join(assembly_dir + '/' + genome, input_file_name)
        print(feature_table)
        ...
        if all(os.path.exists(os.path.join(genome_path, 'analysis', color + '.tsv')) for color in files_to_check):
            ...
            subprocess.run(['python', 'microbiome_main.py', '--genome', genome_path, '-s', str(args.scov), '-q', str(args.qcov), '-r', str(args.autored), '-m', str(args.membrane), '-t', str(args.tmsdiff), '--gbf', gbff_file, '--feattable', feature_table])
