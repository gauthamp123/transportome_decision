    ...
    for key in cleanChebiIDs:
        if cleanChebiIDs[key] == 'none':
            ...
            for id in cleanChebiIDs[key]:
                ...
                myGroup[key]['CE'].append(str(findPredecessor(id,predecessorDict,graph, altIDToID, idToName)) + '-' + getSubstrateName(id, idToName) + '(' + id + ')')
                myGroup[key]['Role'].append(str(findRole(id,idToRelationship,idToName, altIDToID, graph, idToParent,classes=role_classes)) + '-' + getSubstrateName(id, idToName) + '(' + id + ')')
